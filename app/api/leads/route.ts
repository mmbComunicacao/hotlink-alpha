import { generateLead } from '@/app/lib/powercrm'
...
export async function POST(request: Request) {
  ...
  if (name.length < 2 || phone.length < 8) {
    return NextResponse.json(
      { ok: false, message: 'Informe um nome e um telefone válidos.' },
      { status: 422 }
    )
  }
  ...
  try {
    const response = await generateLead({
      name: lead.name,
      phone: lead.phone,
      powercrmId: lead.consultantId
    })

    return NextResponse.json({
      ok: true,
      mode: 'api',
      quotationCode: response.quotationCode
    })
  } catch (error) {
    console.error('Failed to forward lead', error)
    return NextResponse.json(
      { ok: false, message: 'Não foi possível registrar a solicitação.' },
      { status: 502 }
    )
  }
}
