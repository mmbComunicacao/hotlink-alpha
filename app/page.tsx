  ...
  return (
    <div className="relative mx-auto h-28 w-28 sm:h-32 sm:w-32">
      ...
      {consultant.isVerified && (
        <span
          className="absolute bottom-1 right-0 flex h-8 w-8 items-center justify-center rounded-full border-4 border-white bg-teal-600 text-white shadow-md"
...
function CatalogCard({ item }: { item: CatalogItem }) {
  return (
    <a
      ...
      target="_blank"
      rel="noreferrer"
      className="group flex min-h-71.5 flex-col overflow-hidden rounded-[1.35rem] border border-slate-200/80 bg-white shadow-[0_12px_35px_rgba(15,23,42,0.05)] transition duration-200 hover:-translate-y-1 hover:border-teal-200 hover:shadow-[0_18px_45px_rgba(15,118,110,0.13)]"
    >
      <div className="relative h-40 overflow-hidden bg-slate-100">
        ...
        {item.tag && (
          <span className="absolute bottom-3 left-3 rounded-full bg-white/90 px-3 py-1 text-[10px] font-bold uppercase tracking-[0.16em] text-slate-700 backdrop-blur">
            {item.tag}
          </span>
        )}
      </div>
      <div className="flex flex-1 flex-col p-5">
        <div className="flex items-start justify-between gap-3">
          ...
          <span className="mt-0.5 text-teal-600 transition-transform group-hover:translate-x-1">
...
function ContactRow({
  ...
  icon: 'mail' | 'map' | 'clock' | 'instagram' | 'globe'
  ...
  return (
    <main className="min-h-screen overflow-hidden bg-[#FDFCF8]">
      ...
      >
        ...
        <div className="absolute -right-40 top-64 h-136 w-136 rounded-full bg-slate-100/80 blur-3xl" />
      </div>
...
>>>>>>> 2a933924c74e53d1d69de117dd8f29df472e3ade
          ...
          <div className="flex items-center gap-2">
            {source === 'demo' && (
              <span className="hidden rounded-full bg-amber-50 px-3 py-1.5 text-[10px] font-bold uppercase tracking-[0.16em] text-amber-700 sm:inline-flex">
      ...
      <section className="relative z-10 mx-auto max-w-6xl px-5 pb-12 pt-10 sm:px-8 sm:pt-14 lg:px-10 lg:pt-20">
        <div className="grid gap-9 lg:grid-cols-[1.18fr_0.82fr] lg:items-start">
          <div className="rounded-4xl border border-white/80 bg-white/65 p-6 shadow-[0_24px_70px_rgba(15,23,42,0.07)] backdrop-blur sm:p-10 lg:p-12">
            <div className="flex flex-col items-center text-center">
              ...
              <h1 className="mt-3 max-w-xl text-3xl font-semibold tracking-[-0.045em] text-slate-950 sm:text-5xl">
                {consultant.name}
              </h1>
              ...
              <div className="mt-8 flex w-full flex-col gap-3 sm:w-auto sm:flex-row">
                ...
                >
                  <Icon name="whatsapp" size={21} /> Falar comigo no WhatsApp{' '}
          ...
          <aside className="rounded-4xl border border-slate-200/80 bg-white/75 p-6 shadow-[0_24px_70px_rgba(15,23,42,0.05)] backdrop-blur sm:p-8 lg:mt-6">
            <div className="flex items-center justify-between border-b border-slate-100 pb-5">
              <h2 className="text-lg font-semibold tracking-tight text-slate-950">
                Sobre este atendimento
              </h2>
              <span className="rounded-full bg-teal-50 px-3 py-1 text-[10px] font-bold uppercase tracking-[0.14em] text-teal-700">
                Online
              </span>
            </div>
            ...
            <div className="mt-7 space-y-4 border-t border-slate-100 pt-6">
              ...
              <ContactRow icon="mail" href={`mailto:${consultant.email}`}>
                {consultant.email}
              </ContactRow>
              {consultant.instagramUrl && consultant.instagramHandle && (
                <ContactRow icon="instagram" href={consultant.instagramUrl}>
                  {consultant.instagramHandle}
                </ContactRow>
              )}
              {consultant.websiteUrl && (
                <ContactRow icon="globe" href={consultant.websiteUrl}>
        ...
        <section className="mt-16 sm:mt-20" aria-labelledby="catalog-title">
          ...
          <div className="grid gap-5 sm:grid-cols-2 lg:grid-cols-3">
            {consultant.catalog.map(item => (
              <CatalogCard item={item} key={`${consultant.id}-${item.title}`} />
            ))}
          </div>
        </section>
      </section>
