...
const demoCatalog: CatalogItem[] = [
  {
    ...
    description: "Soluções para proteger seu veículo, sua família e seu patrimônio.",
    imageUrl:
  ...
  },
  {
    ...
    description: "Uma oportunidade para consultores ampliarem sua atuação com suporte.",
    imageUrl:
  ...
  },
  {
    ...
    description: "Informação e orientação para escolhas mais seguras em todos os momentos.",
    imageUrl:
  ...
  {
    title: "Grupo MMB",
    description: "Experiência, inovação e compromisso para desenvolver negócios sólidos.",
    imageUrl:
  ...
  },
  {
    ...
    description: "Planeje suas conquistas com acompanhamento para escolher o melhor caminho.",
    imageUrl:
  ...
  {
    title: "Juntos Pod+",
    description: "Conversas que informam, inspiram e conectam pessoas e negócios.",
    imageUrl:
...
function mapCatalog(value: unknown): CatalogItem[] {
  ...
  const mapped = value
    .map((item): CatalogItem | null => {
      ...
      return {
        title,
        description: asString(source.description ?? source.summary, "Conheça esta solução da Alpha."),
        imageUrl: asString(
          source.imageUrl ?? source.image ?? source.thumbnail,
          demoCatalog[0].imageUrl,
        ),
        href: asString(source.href ?? source.url ?? source.link, "https://alphaprotecoes.com.br"),
...
export function normalizeConsultant(payload: unknown, id: string): Consultant {
  ...
  const nested = (source.consultant && typeof source.consultant === "object"
    ? source.consultant
    : source.data && typeof source.data === "object"
      ? source.data
      : source) as UnknownRecord;
  const phone = asString(nested.phone ?? nested.telephone ?? nested.mobile, demoConsultant.phone);
  ...
  return {
    ...
    role: asString(nested.role ?? nested.title ?? nested.cargo, demoConsultant.role),
    tagline: asString(nested.tagline ?? nested.headline ?? nested.slogan, demoConsultant.tagline),
    phone,
    whatsappNumber,
    email: asString(nested.email, demoConsultant.email),
    location: asString(nested.location ?? nested.city ?? nested.cidade, demoConsultant.location),
    address: asString(nested.address ?? nested.endereco, demoConsultant.address),
    description: asString(nested.description ?? nested.bio ?? nested.sobre, demoConsultant.description),
    instagramHandle: asString(nested.instagramHandle ?? nested.instagram ?? nested.instagram_handle, demoConsultant.instagramHandle),
    instagramUrl: asString(nested.instagramUrl ?? nested.instagram_url, demoConsultant.instagramUrl),
    websiteUrl: asString(nested.websiteUrl ?? nested.website ?? nested.site, demoConsultant.websiteUrl),
    hours: asString(nested.hours ?? nested.businessHours ?? nested.horario, demoConsultant.hours),
    avatarUrl: asString(nested.avatarUrl ?? nested.avatar ?? nested.photo ?? nested.foto, demoConsultant.avatarUrl),
    isVerified: asBoolean(nested.isVerified ?? nested.verified, demoConsultant.isVerified),
    catalog: mapCatalog(nested.catalog ?? nested.catalogue ?? nested.products ?? nested.servicos),
  };
}
...
export async function getConsultant(id: string) {
  ...
  if (!apiUrl || !normalizedId) {
    return { consultant: getDemoConsultant(normalizedId || "demo"), source: "demo" as const };
  }
  ...
  try {
    ...
    return { consultant: normalizeConsultant(payload, normalizedId), source: "api" as const };
  } catch (error) {
    console.error("Failed to load consultant from API", error);
    return { consultant: getDemoConsultant(normalizedId), source: "demo" as const };
  }
}

export function getWhatsAppUrl(number: string, consultantName: string) {
  const cleanNumber = number.replace(/\D/g, "");
  const message = encodeURIComponent(`Olá, ${consultantName}! Gostaria de conhecer melhor as soluções da Alpha.`);
