...
export default function LeadBanner({
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      // 1. Faz a requisição (retorna um objeto Response)
      ...
      // 5. Envia a próxima requisição com o quotationCode correto
      const responseActivity = await fetch('/api/activity', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ consultantName, quotationCode })
      })

      if (!responseActivity) console.log('[POWERCRM] Atividade não adicionada')
    } catch (error) {
            ...
            {status === 'success' ? (
              ...
              <form onSubmit={handleSubmit} className="space-y-4">
                ...
                <p className="text-[10px] leading-4 text-slate-400">
                  Usaremos seu contato apenas para retornar esta solicitação.
                </p>
                <button
                  type="submit"
